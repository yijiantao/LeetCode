# -*- coding: UTF-8 -*-
"""
    1. 代理模式：一个类代表另一个类的功能。这种类型的设计模式属于结构型模式。
...
class infoServerProxy(serverProxy):    # 定义infoServerProxy充当Server的直接接口代理
    server = ""
...
class whiteInfoServerProxy(infoServerProxy):    # whiteInfoServerProxy直接继承了infoServerProxy对象，同时加入了white_list和对白名单的操作。
    white_list = []
...
if __name__ == "__main__":
    """
        在场景中通过对白名单代理的访问，就可以实现服务器的白名单访问了。
    """
    info_struct = dict()
